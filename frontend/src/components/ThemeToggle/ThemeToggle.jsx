...
export default function ThemeToggle() {
  ...
  return (
    <>
      {flyPos && (
        <div
          className={`${s.ripple} ${flyPos.goingDark ? s.rippleDark : s.rippleLight}`}
          style={{ left: flyPos.x, top: flyPos.y }}
        />
      )}
      {flyPos && (
        <div
          className={`${s.flyIcon} ${flyPos.goingDark ? s.flySet : s.flyRise}`}
          style={{ left: flyPos.x, top: flyPos.y }}
        >
      ...
      <button
        ref={btnRef}
        className={`${s.btn} ${animating ? s.btnSpin : ''}`}
        onClick={handleClick}
        title={isDark ? 'Chuyển sang sáng' : 'Chuyển sang tối'}
      >
        <span className={animating ? s.iconHide : s.iconShow}>
